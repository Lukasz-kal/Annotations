...
addOverlappingSpans = function (annotations) {
  if (annotations.length > 1) {
    annotations.sort(function (a, b) {
      if (a.target.selector[1].start == b.target.selector[1].start) return 0;
      if (a.target.selector[1].start < b.target.selector[1].start) return -1;
      if (a.target.selector[1].start > b.target.selector[1].start) return 1;
    });
    annotations.forEach((element, index) => {
      if (index < annotations.length - 1) {
        var next = index + 1;
        if (
          annotations[next].target.selector[1].start <=
          element.target.selector[1].end
        ) {
          const [domStart, domEnd] = charOffsetsToDOMPosition([
            annotations[next].target.selector[1].start,
            element.target.selector[1].end,
          ]);
          var range = new Range();
...
charOffsetsToDOMPosition = (charOffsets) => {
  ...
  const textNodeProps = (() => {
    ...
    return walkTextNodes(
      document.getElementById("textToAnnotate"),
      maxOffset
    ).map(function (node) {
